...
// Flatten all jobs for dispatch/history views
const allJobs = () => SEED_COMPANIES.flatMap(co=>co.branches.flatMap(b=>b.agents.flatMap(a=>(a.jobs||[]).map(j=>({...j,agentName:a.name,branchName:b.name,companyName:co.name})))));
...
function JobsSection({agent,onUpdate,jobTypes,onJobTypesChange,technicians,onTechniciansChange}) {
  ...
  const saveJob=()=>{if(!newJob.ref||!newJob.address)return;const type=newJob.type||jobTypes[0]||"";const j={...newJob,type,id:uid(),createdDate:new Date().toISOString().split("T")[0],closedDate:newJob.status==="Closed"?new Date().toISOString().split("T")[0]:null,tenants:[],appliances:[],additionalWorks:[]};onUpdate({...agent,jobs:[...jobs,j]});setNewJob({ref:"",type:"",address:"",description:"",tech:"",keyMethod:"",keyNotes:"",status:"Open"});setShowAddJob(false);};
  const saveTenant=()=>{if(!newTenant.name)return;const t={...newTenant,id:uid()};const updated={...showJob,tenants:[...showJob.tenants,t]};updateJob(updated);setNewTenant({name:"",email:"",phone:""});setShowAddTenant(false);};
  ...
  if(showJob){const js=jobStatus(showJob);return(<div>
    ...
    <Card style={{marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}><div><div style={{color:C.accent,fontWeight:800,fontSize:13}}>{showJob.ref}</div><div style={{color:C.text,fontWeight:800,fontSize:16,marginTop:2}}>{showJob.address}</div></div><Badge label={js} color={statusColor(js)}/></div><Field label="Type" value={showJob.type}/><Field label="Description" value={showJob.description||"—"}/><Field label="Technician" value={showJob.tech||"Unassigned"}/><Field label="Key Access" value={showJob.keyMethod==="tenant"?"🧑 Tenant to give access":showJob.keyMethod==="office"?"🏢 Collect from office":showJob.keyMethod==="other"?"🔑 Other":"—"}/>{showJob.keyNotes&&<Field label="Key Notes" value={showJob.keyNotes}/>}<Field label="Created" value={showJob.createdDate}/>{showJob.closedDate&&<Field label="Closed" value={showJob.closedDate}/>}{showJob.status==="Open"&&<div style={{marginTop:14}}><Btn label="Mark as Closed" onClick={closeJob} color={C.orange} small/></div>}</Card>
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:14,overflowX:"auto",paddingBottom:4}}>{["All","Open","Recently Closed","Old"].map(f=><Pill key={f} label={f} active={filter===f} onClick={()=>setFilter(f)}/>)}</div>
    {filtered.length===0&&<p style={{color:C.muted,fontSize:13,padding:"8px 0"}}>No jobs found.</p>}
    {filtered.map(job=>{const js=jobStatus(job);const apCount=(job.appliances||[]).length,wkCount=(job.additionalWorks||[]).length;return(<RowCard key={job.id} onClick={()=>setShowJob(job)}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}><div style={{flex:1,minWidth:0,marginRight:10}}><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><span style={{color:C.accent,fontWeight:800,fontSize:13}}>{job.ref}</span><Badge label={job.type} color={job.type==="HVAC"?"blue":job.type==="Plumbing"?"purple":"orange"}/></div><div style={{color:C.text,fontSize:13,fontWeight:600,marginTop:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{job.address}</div><div style={{color:C.sub,fontSize:12,marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{job.description}</div><div style={{display:"flex",gap:10,marginTop:6,fontSize:12,color:C.sub,flexWrap:"wrap"}}><span>👷 {job.tech||"Unassigned"}</span><span>👥 {job.tenants.length}</span>{apCount>0&&<span>🏠 {apCount} appliance{apCount!==1?"s":""}</span>}{wkCount>0&&<span>🛠️ {wkCount} work{wkCount!==1?"s":""}</span>}</div></div><Badge label={js} color={statusColor(js)}/></div></RowCard>);})}
    {showAddJob&&(<Modal title="New Job" onClose={()=>setShowAddJob(false)} onSave={saveJob}><FF label="Job Reference" value={newJob.ref} onChange={v=>setNewJob({...newJob,ref:v})} placeholder="e.g. JOB-1007" required/><ListManager label="Job Type" items={jobTypes} onChange={onJobTypesChange}/><div style={{marginBottom:14}}><label style={{display:"block",color:C.sub,fontSize:12,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:5}}>Job Type</label><select value={newJob.type||jobTypes[0]} onChange={e=>setNewJob({...newJob,type:e.target.value})} style={{width:"100%",background:C.raised,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontSize:13,fontFamily:"inherit",boxSizing:"border-box"}}>{jobTypes.map(t=><option key={t}>{t}</option>)}</select></div><FF label="Property Address" value={newJob.address} onChange={v=>setNewJob({...newJob,address:v})} placeholder="e.g. 22 Oak St, Parramatta NSW" required/><FF label="Description" value={newJob.description} onChange={v=>setNewJob({...newJob,description:v})} placeholder="Describe the work needed..." type="textarea"/><ListManager label="Technician" items={technicians} onChange={onTechniciansChange}/><div style={{marginBottom:14}}><label style={{display:"block",color:C.sub,fontSize:12,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:5}}>Assigned Technician</label><select value={newJob.tech} onChange={e=>setNewJob({...newJob,tech:e.target.value})} style={{width:"100%",background:C.raised,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontSize:13,fontFamily:"inherit",boxSizing:"border-box"}}><option value="">— Unassigned —</option>{technicians.map(t=><option key={t}>{t}</option>)}</select></div><div style={{marginBottom:14}}><label style={{display:"block",color:C.sub,fontSize:12,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:5}}>Key Access</label><div style={{display:"flex",flexDirection:"column",gap:6}}>{[{val:"tenant",label:"🧑 Tenant to give access"},{val:"office",label:"🏢 Collect from office"},{val:"other",label:"🔑 Other"}].map(opt=>(<label key={opt.val} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:8,border:`1.5px solid ${newJob.keyMethod===opt.val?C.accent:C.border}`,background:newJob.keyMethod===opt.val?"#eff6ff":"#fff",cursor:"pointer"}}><input type="radio" name="keyMethod" checked={newJob.keyMethod===opt.val} onChange={()=>setNewJob({...newJob,keyMethod:opt.val})} style={{accentColor:C.accent}}/><span style={{fontSize:13,fontWeight:newJob.keyMethod===opt.val?700:500,color:C.text}}>{opt.label}</span></label>))}</div>{newJob.keyMethod&&<textarea value={newJob.keyNotes||""} onChange={e=>setNewJob({...newJob,keyNotes:e.target.value})} placeholder={newJob.keyMethod==="tenant"?"e.g. Call 30 mins prior…":newJob.keyMethod==="office"?"e.g. Ask for Maria at front desk…":"e.g. Lockbox code 4421…"} rows={2} style={{width:"100%",marginTop:8,background:C.raised,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontSize:13,fontFamily:"inherit",resize:"vertical",boxSizing:"border-box"}}/>}</div><div style={{marginBottom:14}}><label style={{display:"block",color:C.sub,fontSize:12,fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,marginBottom:5}}>Status</label><select value={newJob.status} onChange={e=>setNewJob({...newJob,status:e.target.value})} style={{width:"100%",background:C.raised,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.text,fontSize:13,fontFamily:"inherit",boxSizing:"border-box"}}><option>Open</option><option>Closed</option></select></div></Modal>)}
  </div>);
}
...
function DispatchTab() {
  ...
  const typeColor = t=>t==="HVAC"?"blue":t==="Plumbing"?"purple":"orange";
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      <Btn label="+ New Job" color={C.accent}/>
    </div>
    <input placeholder="Search jobs, address, technician…" value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",background:"#fff",border:`1px solid ${C.border}`,borderRadius:9,padding:"10px 14px",color:C.text,fontSize:14,marginBottom:14,fontFamily:"inherit",boxSizing:"border-box"}}/>
    <div style={{display:"flex",gap:8,marginBottom:16,overflowX:"auto",paddingBottom:4}}>
      <Pill label="All" active={filter==="All"} onClick={()=>setFilter("All")}/>
      {techs.map(t=><Pill key={t} label={t} active={filter===t} onClick={()=>setFilter(t)}/>)}
    </div>
    {filter==="All" ? (
      techs.map(tech=>{
...
function HistoryTab() {
  ...
  return(<div>
    <div style={{marginBottom:16}}>
      <h2 style={{fontSize:18,fontWeight:800,color:C.text}}>Job History</h2>
      <p style={{color:C.sub,fontSize:12,marginTop:2}}>{closed.length} completed jobs</p>
    </div>
...
function QuotesTab() {
  ...
  const filtered=quotes.filter(q=>{
    ...
    const mt=!search||q.ref.toLowerCase().includes(search.toLowerCase())||q.client.toLowerCase().includes(search.toLowerCase());
    return ms&&mt;
  });
  ...
  if(sel)return(<div>
    <Breadcrumb items={[{label:"Quotes",fn:()=>setSel(null)},{label:sel.ref}]}/>
    <Card style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
        <div><div style={{color:C.accent,fontWeight:800,fontSize:13}}>{sel.ref}</div><div style={{color:C.text,fontWeight:800,fontSize:18,marginTop:2}}>{sel.client}</div></div>
        <Badge label={sel.status} color={statusCol(sel.status)}/>
      </div>
      <Field label="Contact" value={sel.contact}/>
      <Field label="Date" value={fmtDate(sel.date)}/>
      <Field label="Expiry" value={fmtDate(sel.expiry)}/>
    </Card>
    <Card style={{marginBottom:14}}>
      <SectionHead title="Line Items"/>
      <div style={{borderBottom:`1px solid ${C.border}`,paddingBottom:8,marginBottom:8,display:"grid",gridTemplateColumns:"1fr auto auto auto",gap:8}}>
        {["Description","Qty","Rate","Amount"].map(h=><span key={h} style={{color:C.sub,fontSize:11,fontWeight:700,textTransform:"uppercase"}}>{h}</span>)}
      </div>
      {sel.items.map((item,i)=>(
        <div key={i} style={{display:"grid",gridTemplateColumns:"1fr auto auto auto",gap:8,padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
      ...
      <div style={{display:"flex",justifyContent:"flex-end",marginTop:12,paddingTop:8,borderTop:`2px solid ${C.border}`}}>
        <div style={{textAlign:"right"}}><div style={{color:C.sub,fontSize:12,fontWeight:600}}>TOTAL</div><div style={{color:C.text,fontWeight:900,fontSize:22}}>{fmtMoney(sel.total)}</div></div>
      </div>
    </Card>
    <div style={{display:"flex",gap:10}}>
      {sel.status==="Draft"&&<Btn label="Send Quote" color={C.accent}/>}
      {sel.status==="Sent"&&<Btn label="Mark Accepted" color={C.green}/>}
      <Btn label="Convert to Invoice" color={C.purple} outline/>
    </div>
  </div>);

  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <div><h2 style={{fontSize:18,fontWeight:800,color:C.text}}>Quotes</h2><p style={{color:C.sub,fontSize:12,marginTop:2}}>{quotes.length} quotes total</p></div>
      <Btn label="+ New Quote" color={C.purple}/>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
      <StatCard label="Pending" value={fmtMoney(totalPending)} sub={`${quotes.filter(q=>q.status==="Sent").length} quotes sent`} icon="📤" color={C.accent}/>
      <StatCard label="Accepted" value={fmtMoney(totalAccepted)} sub={`${quotes.filter(q=>q.status==="Accepted").length} quotes`} icon="✅" color={C.green}/>
    </div>
    <input placeholder="Search quotes…" value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",background:"#fff",border:`1px solid ${C.border}`,borderRadius:9,padding:"10px 14px",color:C.text,fontSize:14,marginBottom:12,fontFamily:"inherit",boxSizing:"border-box"}}/>
    <div style={{display:"flex",gap:8,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
      {["All","Draft","Sent","Accepted","Expired"].map(s=><Pill key={s} label={s} active={statusFilter===s} onClick={()=>setStatusFilter(s)}/>)}
    </div>
    {filtered.map(q=>(
      <RowCard key={q.id} onClick={()=>setSel(q)}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{flex:1,minWidth:0,marginRight:10}}>
            <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
              <span style={{color:C.accent,fontWeight:800,fontSize:13}}>{q.ref}</span>
              <Badge label={q.status} color={statusCol(q.status)}/>
            </div>
            <div style={{color:C.text,fontWeight:700,fontSize:14,marginTop:4}}>{q.client}</div>
            <div style={{color:C.sub,fontSize:12,marginTop:2}}>Contact: {q.contact} · Expires {fmtDate(q.expiry)}</div>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{color:C.text,fontWeight:900,fontSize:16}}>{fmtMoney(q.total)}</div>
            <div style={{color:C.muted,fontSize:11,marginTop:2}}>{q.items.length} items</div>
          </div>
        </div>
      </RowCard>
    ))}
...
function InvoicesTab() {
  ...
  const filtered=invoices.filter(i=>{
    ...
    const mt=!search||i.ref.toLowerCase().includes(search.toLowerCase())||i.client.toLowerCase().includes(search.toLowerCase());
    return ms&&mt;
  });
  ...
  if(sel)return(<div>
    <Breadcrumb items={[{label:"Invoices",fn:()=>setSel(null)},{label:sel.ref}]}/>
    <Card style={{marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
        <div><div style={{color:C.accent,fontWeight:800,fontSize:13}}>{sel.ref}</div><div style={{color:C.text,fontWeight:800,fontSize:18,marginTop:2}}>{sel.client}</div></div>
        <Badge label={sel.status} color={statusCol(sel.status)}/>
      </div>
      <Field label="Job Ref" value={sel.jobRef}/>
      <Field label="Contact" value={sel.contact}/>
      <Field label="Invoice Date" value={fmtDate(sel.date)}/>
      <Field label="Due Date" value={fmtDate(sel.due)}/>
      {sel.paidDate&&<Field label="Paid Date" value={fmtDate(sel.paidDate)}/>}
    </Card>
    <Card style={{marginBottom:14}}>
      <SectionHead title="Line Items"/>
      <div style={{borderBottom:`1px solid ${C.border}`,paddingBottom:8,marginBottom:8,display:"grid",gridTemplateColumns:"1fr auto auto auto",gap:8}}>
    ...
    {sel.status!=="Paid"&&<Btn label="Mark as Paid" color={C.green}/>}
  </div>);

  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <div><h2 style={{fontSize:18,fontWeight:800,color:C.text}}>Invoices</h2><p style={{color:C.sub,fontSize:12,marginTop:2}}>{invoices.length} invoices</p></div>
      <Btn label="+ New Invoice" color={C.green}/>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
      <StatCard label="Outstanding" value={fmtMoney(totalOwing)} sub={`${invoices.filter(i=>i.status!=="Paid").length} unpaid`} icon="⏳" color={C.red}/>
      <StatCard label="Collected" value={fmtMoney(totalPaid)} sub={`${invoices.filter(i=>i.status==="Paid").length} paid`} icon="💰" color={C.green}/>
    </div>
    <input placeholder="Search invoices…" value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",background:"#fff",border:`1px solid ${C.border}`,borderRadius:9,padding:"10px 14px",color:C.text,fontSize:14,marginBottom:12,fontFamily:"inherit",boxSizing:"border-box"}}/>
    <div style={{display:"flex",gap:8,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
      {["All","Draft","Sent","Overdue","Paid"].map(s=><Pill key={s} label={s} active={statusFilter===s} onClick={()=>setStatusFilter(s)}/>)}
    </div>
    {filtered.map(inv=>(
      <RowCard key={inv.id} onClick={()=>setSel(inv)}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{flex:1,minWidth:0,marginRight:10}}>
            <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
              <span style={{color:C.accent,fontWeight:800,fontSize:13}}>{inv.ref}</span>
              <Badge label={inv.status} color={statusCol(inv.status)}/>
            </div>
            <div style={{color:C.text,fontWeight:700,fontSize:14,marginTop:4}}>{inv.client}</div>
            <div style={{color:C.sub,fontSize:12,marginTop:2}}>Job: {inv.jobRef} · Due {fmtDate(inv.due)}</div>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{color:inv.status==="Overdue"?C.red:C.text,fontWeight:900,fontSize:16}}>{fmtMoney(inv.total)}</div>
            <div style={{color:C.muted,fontSize:11,marginTop:2}}>{inv.items.length} items</div>
          </div>
        </div>
      </RowCard>
    ))}
...
function InventoryTab() {
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <div><h2 style={{fontSize:18,fontWeight:800,color:C.text}}>Inventory</h2><p style={{color:C.sub,fontSize:12,marginTop:2}}>{inventory.length} items tracked</p></div>
      <Btn label="+ Add Item" color={C.purple}/>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16}}>
      <StatCard label="Total Items" value={inventory.length} icon="📦" color={C.accent}/>
      <StatCard label="Low Stock" value={lowStock.length} sub="at or below minimum" icon="⚠️" color={C.orange}/>
    ...
    {lowStock.length>0&&(
      <div style={{background:"#fef9c3",border:"1px solid #fde047",borderRadius:10,padding:"12px 14px",marginBottom:14}}>
        <div style={{fontWeight:700,fontSize:13,color:"#854d0e",marginBottom:6}}>⚠️ Low Stock Alert</div>
        {lowStock.map(i=><div key={i.id} style={{fontSize:12,color:"#92400e",marginTop:3}}>• {i.name} — {i.qty} remaining (min {i.minQty})</div>)}
      </div>
    )}
    <input placeholder="Search inventory…" value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",background:"#fff",border:`1px solid ${C.border}`,borderRadius:9,padding:"10px 14px",color:C.text,fontSize:14,marginBottom:12,fontFamily:"inherit",boxSizing:"border-box"}}/>
    <div style={{display:"flex",gap:8,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
    ...
    {filtered.map(item=>(
      <RowCard key={item.id}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{flex:1,minWidth:0,marginRight:10}}>
            <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
              <span style={{color:C.accent,fontWeight:700,fontSize:11}}>{item.sku}</span>
              <Badge label={item.category} color="blue"/>
              <Badge label={item.qty===0?"Out of Stock":item.qty<=item.minQty?"Low Stock":"In Stock"} color={stockColor(item)}/>
            </div>
            <div style={{color:C.text,fontWeight:700,fontSize:14,marginTop:4}}>{item.name}</div>
            <div style={{display:"flex",gap:10,marginTop:4,fontSize:12,color:C.sub,flexWrap:"wrap"}}>
          ...
          <div style={{textAlign:"right",flexShrink:0}}>
            <div style={{color:stockColor(item)==="red"?C.red:stockColor(item)==="orange"?C.orange:C.green,fontWeight:900,fontSize:24}}>{item.qty}</div>
            <div style={{color:C.muted,fontSize:11,marginTop:2}}>{fmtMoney(item.unitCost)} each</div>
          </div>
        </div>
      </RowCard>
    ))}
...
function ReportsTab() {
  ...
  return(<div>
    <div style={{marginBottom:16}}><h2 style={{fontSize:18,fontWeight:800,color:C.text}}>Reports</h2><p style={{color:C.sub,fontSize:12,marginTop:2}}>Business overview</p></div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
      <StatCard label="Total Revenue" value={fmtMoney(revenue)} sub="from paid invoices" icon="💰" color={C.green}/>
      <StatCard label="Outstanding" value={fmtMoney(outstanding)} sub="unpaid invoices" icon="⏳" color={C.red}/>
      <StatCard label="Open Jobs" value={openJobs.length} sub="awaiting completion" icon="🔓" color={C.orange}/>
      <StatCard label="Jobs Completed" value={closedJobs.length} sub="all time" icon="✅" color={C.accent}/>
    </div>
    <Card style={{marginBottom:16}}>
      <SectionHead title="📊 Jobs by Type"/>
      {jobsByType.map(jt=>(
        <div key={jt.type} style={{marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
            <span style={{fontSize:13,fontWeight:600,color:C.text}}>{jt.type}</span>
            <span style={{fontSize:13,fontWeight:700,color:C.sub}}>{jt.count} jobs</span>
          </div>
          <div style={{background:C.raised,borderRadius:99,height:8,overflow:"hidden"}}>
            <div style={{background:jt.type==="HVAC"?C.accent:jt.type==="Plumbing"?C.purple:C.orange,height:"100%",borderRadius:99,width:`${jobs.length?Math.round(jt.count/jobs.length*100):0}%`,transition:"width 0.5s"}}/>
          </div>
        </div>
      ))}
    </Card>
    <Card style={{marginBottom:16}}>
      <SectionHead title="👷 Technician Workload"/>
      {jobsByTech.map(jt=>(
        <div key={jt.tech} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
          <Avatar name={jt.tech} size={34} bg="#dbeafe" fg="#1d4ed8"/>
          <div style={{flex:1}}><div style={{color:C.text,fontWeight:700,fontSize:13}}>{jt.tech}</div><div style={{color:C.sub,fontSize:12}}>{jt.open} open · {jt.closed} completed</div></div>
          <div style={{display:"flex",gap:6}}><Badge label={`${jt.open} open`} color="blue"/><Badge label={`${jt.closed} done`} color="green"/></div>
        </div>
      ))}
    </Card>
    <Card>
      <SectionHead title="📋 Invoice Summary"/>
      {[["Paid",invoices.filter(i=>i.status==="Paid"),"green"],["Overdue",invoices.filter(i=>i.status==="Overdue"),"red"],["Sent",invoices.filter(i=>i.status==="Sent"),"blue"]].map(([label,items,color])=>(
        <div key={label} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",gap:10,alignItems:"center"}}><Badge label={label} color={color}/><span style={{fontSize:12,color:C.sub}}>{items.length} invoice{items.length!==1?"s":""}</span></div>
          <span style={{fontWeight:700,color:C.text}}>{fmtMoney(items.reduce((s,i)=>s+i.total,0))}</span>
        </div>
      ))}
    </Card>
  </div>);
    ...
    <div style={{display:"flex",minHeight:"100vh",background:C.bg,fontFamily:"'Inter','Segoe UI',sans-serif"}}>
      ...
      {!isMobile&&(
        <div style={{width:230,background:C.sidebar,display:"flex",flexDirection:"column",flexShrink:0,minHeight:"100vh",position:"sticky",top:0,height:"100vh",overflowY:"auto"}}>
          ...
          <div style={{padding:"12px 10px",borderTop:"1px solid #334155",flexShrink:0}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:isMobile?"16px 14px 80px":"28px 32px",maxWidth:"100%"}}>
        {isMobile&&(
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            ...
            <div style={{color:C.sub,fontSize:12}}>/ {ALL_NAV.find(n=>n.id===tab)?.label}</div>
          </div>
        )}
